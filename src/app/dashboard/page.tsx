...
import {
  getCurrentBusiness, getBusinessFeatures,
  getDashboardStats, getSalesOverviewData,
...
export default async function DashboardPage({ searchParams }: DashboardPageProps) {
  ...
  const [liveStats, salesData] = await Promise.all([
    hasBot   ? getDashboardStats()      : Promise.resolve(null),
    hasSales ? getSalesOverviewData()   : Promise.resolve(null),
  ]);
  ...
  const todayInvs  = salesData
    ? (salesData.invoices as { issued_at: string; total_amount: number | string; vat_amount: number | string }[])
        .filter((i) => i.issued_at.startsWith(today))
    : [];
  const todayTotal = todayInvs.reduce((s, i) => s + Number(i.total_amount), 0);

  const salesCards = salesData && !salesData.schemaMissing
    ? [
  ...
  /* ── Quick actions (filtered by features) ── */
  const allQuickActions = [
    { label: "إصدار فاتورة",       sub: "فاتورة QR وضريبة ZATCA",   Icon: ReceiptText,       href: "/dashboard/sales/products", color: "#7a5af8", show: hasSales },
    { label: "قائمة الفواتير",     sub: "عرض الفواتير المحفوظة",     Icon: FileText,          href: "/dashboard/sales",          color: "#14b8a6", show: hasSales },
    { label: "استعراض المحادثات", sub: "عرض جميع محادثات العملاء",   Icon: MessageSquareText, href: "/dashboard/conversations",  color: "#7a5af8", show: hasBot   },
    { label: "إعدادات الفوترة",   sub: "الضريبة وبيانات البائع",     Icon: Settings2,         href: "/dashboard/sales/settings", color: "#f59e0b", show: hasSales },
  ].filter((a) => a.show);

  /* ── Hero subtitle ── */
  const heroSub = hasBot && hasSales
    ? "ملخص الفواتير والمبيعات اليوم"
    : hasBot
      ? "ملخص المحادثات والعملاء اليوم"
      : "ملخص المبيعات والفواتير اليوم";

  const heroBiz = hasBot && hasSales
    ? `${business?.name ?? ""} · المبيعات والفوترة`
    : hasBot
      ? `${business?.name ?? ""} · واتساب ذكي`
      : `${business?.name ?? ""} · المبيعات والفوترة`;
    ...
    <div className="grid gap-8">
      ...
      <section className="relative overflow-hidden rounded-2xl px-6 py-5 border transition-colors
        ...
        <div className="relative flex items-center justify-between">
          <span className="text-sm text-surface-500 dark:text-surface-400">{heroBiz}</span>
            ...
            {waCards.map((card) => (
              ...
              >
                <div className="mb-3 flex items-center justify-between">
                  ...
                  <div className="flex h-9 w-9 items-center justify-center rounded-xl" style={{ background: card.color + "26" }}>
            ...
            {salesCards.map((card) =>
              card.isButton ? (
                <Link
                  ...
                  style={{ borderTop: `2px solid ${card.color}`, boxShadow: `0 4px 12px ${card.color}22` }}
                >
                  <div className="mb-4 flex items-center justify-between">
                    <span className="text-[10px] font-semibold" style={{ color: card.color }}>افتح ←</span>
                    <div className="flex h-9 w-9 items-center justify-center rounded-xl" style={{ background: card.color + "26" }}>
                      <card.Icon className="size-[18px]" style={{ color: card.color }} />
                    </div>
                  </div>
                  <div className="flex justify-end">
                    <span className="rounded-xl px-3 py-2 text-sm font-black" style={{ background: card.color + "33", color: card.color, border: `1px solid ${card.color}66` }}>
                      فتح الكاشير
                    </span>
                  </div>
        ...
        <section>
          ...
          <div className="space-y-2.5">
            {allQuickActions.map((action) => (
              ...
              >
                <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 shrink-0 text-surface-400 dark:text-surface-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                ...
                <div className="flex flex-1 items-center justify-end gap-3">
                  ...
                  <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl" style={{ background: action.color + "26" }}>
