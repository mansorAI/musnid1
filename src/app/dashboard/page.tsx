...
export default async function DashboardPage({ searchParams }: DashboardPageProps) {
  ...
  const todayVat     = todayInvs.reduce((s, i) => s + Number(i.vat_amount),   0);
  const todayTotal   = todayInvs.reduce((s, i) => s + Number(i.total_amount), 0);
  const netSalesNum  = todayTotal - todayVat;

  const salesCards = [
    { label: "مبيعات اليوم",    value: `${netSalesNum.toFixed(0)} ر.س`, color: "#14b8a6", Icon: Banknote,    isButton: false, href: "" },
  ...
  return (
    <div className="grid gap-8">
      ...
      {/* ── Welcome hero ── */}
      <section className="relative overflow-hidden rounded-2xl border border-surface-700 bg-surface-900 px-6 py-5">
        <div className="pointer-events-none absolute -right-10 -top-20 h-56 w-56 rounded-full bg-primary-500 opacity-[0.07]" />
        ...
        <p className="relative mt-2 text-right text-sm text-surface-400">
      ...
      {waCards ? (
        <section>
          <p className="mb-3 text-right text-xs font-semibold uppercase tracking-wider text-surface-500">
            واتساب ذكي
          </p>
          <div className="grid grid-cols-2 gap-3 xl:grid-cols-4">
            {waCards.map((card) => (
              <div
                key={card.label}
                className="rounded-2xl border border-surface-700/50 bg-surface-800/40 p-4 dark:bg-surface-800/30"
              ...
              >
                ...
                <p className="mt-1.5 text-right text-xs text-surface-500">{card.label}</p>
              </div>
            ))}
          </div>
        </section>
            ...
            {salesCards.map((card) =>
              card.isButton ? (
                <Link
                  key={card.label}
                  href={card.href}
                  className="block rounded-2xl border border-surface-700/50 bg-surface-800/40 p-4 transition-all duration-200 hover:-translate-y-0.5 hover:shadow-lg dark:bg-surface-800/30"
                  style={{ borderTopWidth: 2, borderTopColor: card.color }}
                >
                  <div className="mb-3 flex items-center justify-between">
                  ...
                  <p className="mt-3 text-right text-xs text-surface-500">{card.label}</p>
                </Link>
              ) : (
                <div
                  key={card.label}
        ...
        <div className="space-y-2.5">
          {quickActions.map((action) => (
            <Link
              key={action.href}
              href={action.href}
              className="flex items-center rounded-2xl border border-surface-700 bg-surface-900 px-4 py-3.5 transition-colors hover:bg-surface-800"
            >
              {/* Chevron on far left */}
              <svg
              ...
              {/* Text + icon on far right */}
              <div className="flex flex-1 items-center justify-end gap-3">
                <div className="text-right">
                  <p className="text-sm font-semibold text-white">{action.label}</p>
                  <p className="mt-0.5 text-xs text-surface-500">{action.sub}</p>
                </div>
                <div
