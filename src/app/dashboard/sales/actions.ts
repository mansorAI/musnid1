...
export async function saveProductDisplayConfig(formData: FormData) {
  ...
  const cfg = {
    ...
    allowRepeatedDateBookings: formData.get("allowRepeatedDateBookings") === "on",
    allowRepeatedTimeBookings: formData.get("allowRepeatedTimeBookings") === "on",
    paymentTimeoutMinutes: Math.max(5, Math.min(1440, Number(formData.get("paymentTimeoutMinutes") ?? 30) || 30)),
