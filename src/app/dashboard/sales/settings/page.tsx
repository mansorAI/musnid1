...
import { getInvoiceSettingsData, getProductDisplayData } from "@/lib/dashboard-data";
import { saveInvoiceSettings } from "../actions";
import { DisplayConfigForm } from "./display-config-form";
...
export default async function SalesSettingsPage({ searchParams }: SalesSettingsPageProps) {
  ...
  const tab = params?.tab === "display" ? "display" : "invoice";

  const [invoiceData, displayData] = await Promise.all([
    tab === "invoice" ? getInvoiceSettingsData() : Promise.resolve(null),
    tab === "display" ? getProductDisplayData() : Promise.resolve(null),
  ]);

  return (
    <div className="grid gap-6">
      <section>
        <p className="text-sm text-surface-500 dark:text-surface-400">إعدادات المبيعات</p>
        <h1 className="text-2xl font-extrabold text-surface-900 dark:text-white">الإعدادات</h1>
      </section>

      {/* ── تاب بار ── */}
      <nav className="flex gap-2 rounded-2xl border border-surface-200 bg-white p-1.5 dark:border-surface-800 dark:bg-surface-900">
        {[
          { id: "invoice", label: "الفوترة والضريبة" },
          { id: "display", label: "إعداد عرض المنتج" },
        ].map((t) => (
          <Link
            key={t.id}
            href={`/dashboard/sales/settings${t.id === "invoice" ? "" : `?tab=${t.id}`}`}
          ...
          <div className="glass-card overflow-hidden">
            ...
            <form action={saveInvoiceSettings} className="space-y-4 p-6">
              ...
              <label className="block space-y-1.5 text-sm font-medium text-surface-700 dark:text-surface-300">
                اسم البائع
                <input name="seller_name" defaultValue={invoiceData.settings.sellerName} className="w-full rounded-xl border border-surface-200 bg-surface-50 px-4 py-3 dark:border-surface-700 dark:bg-surface-800/50" />
              </label>
              <label className="block space-y-1.5 text-sm font-medium text-surface-700 dark:text-surface-300">
                الرقم الضريبي
                <input name="vat_number" dir="ltr" maxLength={15} defaultValue={invoiceData.settings.vatNumber} placeholder="300000000000003" className="w-full rounded-xl border border-surface-200 bg-surface-50 px-4 py-3 dark:border-surface-700 dark:bg-surface-800/50" />
              </label>
              <label className="block space-y-1.5 text-sm font-medium text-surface-700 dark:text-surface-300">
                عنوان البائع
              ...
              <label className="flex items-center gap-2 text-sm text-surface-700 dark:text-surface-300">
                <input name="vat_registered" type="checkbox" className="size-4" defaultChecked={Boolean(invoiceData.settings.vatNumber)} />
                مسجل في ضريبة القيمة المضافة
              </label>
              <label className="block space-y-1.5 text-sm font-medium text-surface-700 dark:text-surface-300">
                طريقة الضريبة
                <select name="vat_mode" defaultValue={invoiceData.settings.taxMode} className="w-full rounded-xl border border-surface-200 bg-surface-50 px-4 py-3 dark:border-surface-700 dark:bg-surface-800/50">
      ...
      {/* ── تاب إعداد عرض المنتج ── */}
      {tab === "display" && displayData && (
        <div className="max-w-xl">
          <p className="mb-5 text-sm text-surface-500 dark:text-surface-400">
            اختر ما يظهر على كارت المنتج في الكاشير وزون. التغييرات تؤثر على الموبايل والويب معاً.
          </p>
          <DisplayConfigForm
            config={displayData.config}
            constraints={displayData.constraints}
            saved={params?.saved === "1" && params?.tab === "display"}
          />
        </div>
      )}
    </div>
