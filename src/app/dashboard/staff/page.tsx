...
const demoStaff = [
  { id: "demo-s1", name: "د. أحمد الراشدي", title: "طبيب عام", specialty: "طب عام", is_active: true, display_order: 1 },
  { id: "demo-s2", name: "سارة الغامدي", title: "ممرضة", specialty: null, is_active: true, display_order: 2 },
];
...
export default async function StaffPage({ searchParams }: StaffPageProps) {
  ...
  const staff = rawStaff.length === 0 ? demoStaff : rawStaff;
  const isDemo = rawStaff.length === 0;

  return (
    <div className="grid gap-6 lg:grid-cols-[1fr_360px]">
      <section className="space-y-2 lg:col-span-2">
        <p className="text-sm text-surface-500 dark:text-surface-400">الفريق</p>
        <h1 className="text-2xl font-extrabold text-surface-900 dark:text-white">أعضاء الفريق</h1>
      </section>

      <div className="glass-card overflow-hidden">
        <div className="flex items-center justify-between p-6 border-b border-surface-200/50 dark:border-surface-700/30">
          <div>
            <h2 className="text-lg font-bold text-surface-900 dark:text-white">قائمة الفريق</h2>
            {isDemo && (
      ...
      <div className="glass-card overflow-hidden">
        <div className="p-6 border-b border-surface-200/50 dark:border-surface-700/30">
          <h2 className="text-lg font-bold text-surface-900 dark:text-white">إضافة عضو</h2>
        </div>
        <div className="p-6">
          {params?.error && (
